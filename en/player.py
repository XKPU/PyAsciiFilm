...
def play_video(video_path, use_color=False):
    ...
    try:
        while True:
            ...
            # Information output
            color_mode_text = "Color" if use_color else "Grayscale"
            progress_info = f"Average FPS: {avg_fps:.1f} | Frame: {displayed_frame}/{total_frames} | {color_mode_text}"
        ...
        print("\n\n[Exit] Playback interrupted by user")
        return True
    finally:
        # Stop subprocess
        stop_event.set()
